...
#if 1
/*****前面三行是对分频锁存器进行配置，配置设置为115200******/
    UART_LCR(uart_dev->base) = 0x83;
    UART_LSB(uart_dev->base) = 0x44;		
...
{
    unsigned char static FRC,LCR,MCR,FRC0,LCR0,MCR0,LSB0,MSB0,LSB,MSB,DAT,IER,DAT0,IER0;
...
struct serial_configure serial4_cfg=
{
...
void rt_hw_uart_init(void)
{
    ...
    serial4.ops    = &mipssim_uart_ops;
    serial4.config = config;

    rt_hw_interrupt_install(uart->IRQ, uart_irq_handler, &serial4, "UART4");
   // mipssim_uart_configure(uart,&serial4_cfg);
    /* register UART device */
